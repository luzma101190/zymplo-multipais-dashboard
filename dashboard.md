...
| 🇪🇸 **España** | ✅ **E2E QA público live 2026-05-19** (provider mock · cert AEAT pendiente Jesús) | ✅ **E2E Tink REAL Demo Bank 2026-05-20** (Tink Console creds activos · 19 cuentas Demo Bank ES/EU + 1 tx fetched directo contra api.tink.com) · link 100 en `ZMP_OF_LINK provider=tink` con access_token fresh · persistencia full ZMP_OF_ACCOUNT/TRANSACTION pendiente Oracle pool fix (.env runtime DB_CONNECT_STRING) | ✅ App · ✅ WA (PR #843 mobile hook + #849 bot tools + workflow + mount proxy) | [facturacion-es-qa](https://facturacion-es-qa.zymplo.com/api-docs/) · [openfinance-es-qa](https://openfinance-es-qa.zymplo.com/api-docs/) |
...
- **Pendiente para persist via backend (refresh+sync)**: container `of-es-qa` reportó ORA-12514 post último redeploy · necesita Jesús verificar `.env runtime DB_CONNECT_STRING` (descriptor TCP completo con `MY_WALLET_DIRECTORY` embebido vs alias TNS solo) + `docker compose up -d --force-recreate` · una vez pool reconnectado, `POST /links/100/refresh` con `x-of-service-token` va a leer providerExtra (CLOB fix #904), llamar Tink con token vigente, y persistir 19 cuentas en `ZMP_OF_ACCOUNT country=ESP provider=tink` automáticamente
