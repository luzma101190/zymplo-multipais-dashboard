...
| 🇪🇸 **España** | ✅ **E2E con Oracle real validado 2026-05-19** (provider mock · cert AEAT pendiente Jesús) | ❌ no aplica (no aggregator EU integrado) | ✅ App · ✅ WA (PR #843 mobile hook + #849 bot tools + workflow + mount proxy) | externo + en monorepo `espana/zymplo-aeat/` · [PDF](samples/espana/factura-21-es-E2E-2.pdf) · [Facturae XML](samples/espana/facturae-21-es-E2E-2.xml) |
...
- **Sample E2E (Oracle real)**: [📄 PDF factura](samples/espana/factura-21-es-E2E-2.pdf) (101KB · A4 · 1 página · contiene los elementos legales obligatorios de Ley 37/1992 IVA + RD 1619/2012 + QR Verifactu RD 1007/2023 con URL `prewww2.aeat.es` · ⚠️ AEAT NO define layout PDF oficial · lo oficial obligatorio es el XML Facturae 3.2.2 abajo + el QR Verifactu) · [📋 Facturae XML 3.2.2](samples/espana/facturae-21-es-E2E-2.xml) (4947 bytes · namespace `http://www.facturae.es/Facturae/2014/v3.2.2/Facturae` · ESTE es el formato oficial obligatorio) · [HTML preview](samples/espana/factura-21-es-E2E-2.html)
...
- **Estado runtime QA**: backend probado E2E contra Oracle real desde migrations VM 2026-05-19 (luzes@137.131.217.98) · template en `servers/nfe-s/facturacion-es-qa/` listo (commit infra 243ae48) · pendiente DevOps deploy público en `facturacion-es-qa.zymplo.com` · faltan (1) cert AEAT real (Jesús · gestión externa) · (2) activar `FE_ES_ENABLED=true` en zymplo-api `.env` · (3) instalar Chromium ARM64 en runner para PDF nativo (o aceptar `FE_ES_PDF_FALLBACK=true` con render post-facto) · (4) confirmar DB user TMP_CLAUDE provisional aceptable o usar `BILLING_QA_APP`
